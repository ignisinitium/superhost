...
async function handleTask(task) {
    ...
    try {
        ...
        await client.query('UPDATE tasks SET status = \'failed\', error_message = $1, updated_at = NOW() WHERE id = $2', [err.message, task.id]);
    }
}
...
async function handleReadLogs(payload, taskId) {
    ...
    switch (logType) {
        ...
        case 'nginx_error':
            ...
            break;
        case 'php_fpm':
            filePath = '/var/log/php8.5-fpm.log';
            break;
        case 'system':
...
async function handleGenerateEmailDns(payload) {
    ...
    try {
        ...
        // SPF
        await client.query('INSERT INTO domain_dns_records (domain_id, type, name, content) VALUES ($1, $2, $3, $4)', [domainId, 'TXT', '@', 'v=spf1 mx a -all']);
        // DKIM
        await client.query('INSERT INTO domain_dns_records (domain_id, type, name, content) VALUES ($1, $2, $3, $4)', [domainId, 'TXT', `${selector}._domainkey`, dkimRecord]);
        // DMARC
        await client.query('INSERT INTO domain_dns_records (domain_id, type, name, content) VALUES ($1, $2, $3, $4)', [domainId, 'TXT', '_dmarc', `v=DMARC1; p=quarantine; sp=quarantine; adkim=r; aspf=r;`]);
...
async function handleInstallWordPress(payload) {
    ...
    if (!domainName || !username || !dbName)
        throw new Error('Missing required fields for WordPress installation');
...
async function handleCheckNodeHealth(payload) {
    ...
    if (!nodeId || !ipAddress)
        throw new Error('nodeId and ipAddress are required');
    try {
