...
`;
        ...
        try {
            ...
            await execPromise(`sudo certbot --nginx ${await certbotDomainArgs(domainName)} --expand --non-interactive --agree-tos --email ${shellEscape(certbotEmail)}`);
...
async function certbotDomainArgs(domain) {
...
async function handleInstallSsl(payload) {
    ...
    await execPromise(`certbot --nginx ${await certbotDomainArgs(domainName)} --expand --non-interactive --agree-tos --email ${shellEscape(certbotEmail)}`);
    await client.query('UPDATE domains SET is_ssl = TRUE WHERE domain_name = $1', [domainName]);
...
async function regenerateMailSni() {
    const script = `set -e
...
async function provisionMailconfigVhost(domainName) {
    ...
    if (!hasCert)
        return; // the mail cert (covering autoconfig/autodiscover SANs) must exist first
...
async function handleProvisionWebmailVhost(payload) {
...
`;
    ...
    if (!gotCert) {
        ...
        if (retry < 8) {
            await client.query('INSERT INTO tasks (command, payload) VALUES ($1, $2)', ['PROVISION_WEBMAIL_VHOST', { domainName, retry: retry + 1 }]);
        }
...
async function resolveMaildirPath(filePath) {
...
function cleanReleasedMessage(buf) {
    const text = buf.toString('latin1');
    const m = text.match(/\r?\n\r?\n/);
    if (!m || m.index === undefined)
        return buf; // no header/body boundary — leave as-is
    ...
    for (const line of head.split(/\r?\n/)) {
        ...
        if (droppingFolded && isFolded)
            continue; // drop continuation of a removed header
        droppingFolded = false;
        if (/^X-Spam-/i.test(line)) {
            droppingFolded = true;
            continue;
        }
        const subj = line.match(/^(Subject:\s*)\[SPAM\]\s*(.*)$/i);
        if (subj) {
            out.push(`${subj[1]}${subj[2]}`);
            continue;
        }
        out.push(line);
    }
...
async function handleReleaseQuarantine(payload) {
    ...
    try {
        ...
        let cleaned = false;
        try {
            const { stdout } = await execPromise(`sudo cat ${shellEscape(srcPath)}`, { maxBuffer: 50 * 1024 * 1024, encoding: 'buffer' });
            const rawBuf = Buffer.isBuffer(stdout) ? stdout : Buffer.from(stdout);
            ...
            console.warn(`Release clean-up failed for ${id}, moving as-is: ${e.message}`);
...
async function handleReadQuarantineMessage(payload, taskId) {
    const { quarantineId } = payload;
    if (!quarantineId)
        throw new Error('quarantineId required');
    ...
    if (!filePath)
        throw new Error('Quarantine entry not found');
    ...
    const { stdout } = await execPromise(`sudo cat ${shellEscape(realPath)}`, { maxBuffer: 25 * 1024 * 1024, encoding: 'buffer' });
    const rawBuf = Buffer.isBuffer(stdout) ? stdout : Buffer.from(stdout);
    ...
    await client.query("UPDATE tasks SET status = 'completed', payload = payload || $1, updated_at = NOW() WHERE id = $2", [JSON.stringify({ result }), taskId]);
}
/**
...
async function detectSiteApp(docRoot) {
...
`;
    ...
    try {
        ...
        return JSON.parse(stdout.trim() || '{"type":"static"}');
    }
    catch {
        return { type: 'static' };
    }
...
const mysqlSafe = (s) => (s || '').toLowerCase().replace(/[^a-z0-9_]/g, '_').replace(/^([0-9])/, '_$1');
// Dump a remote database (using the app's own credentials) straight into a
// freshly-created local database over the SSH pipe.
async function importRemoteDbWithCreds(host, port, remoteUser, cred, dbHost, dbUser, dbPass, remoteDb, localDb, localDbUser, localDbPass) {
...
async function migrateSiteDb(sshHost, sshPort, sshUser, cred, userId, username, db) {
    ...
    await client.query(`INSERT INTO databases (user_id, db_name, db_user) VALUES ($1,$2,$3) ON CONFLICT (db_name) DO NOTHING`, [userId, localDb, localUser]);
    return { localDb, localUser, localPass };
}
// Rewrite the site's config to point at the new local DB (localhost).
async function rewriteSiteDbConfig(type, configFile, localDb, localUser, localPass) {
...
async function handleMigrateSite(payload) {
    ...
    if (direction === 'push') {
        try {
            ...
            await rsyncToRemote(sourceHost, Number(sourcePort), sshUser, cred, docRoot + '/', String(remotePath).replace(/\/?$/, '/'), ['--exclude=node_modules', '--exclude=.git', '--exclude=.env']);
            await siteLog(migrationId, 'Files uploaded.');
            // Optional post-deploy commands run ON the remote server (install / build / restart).
            for (const [label, cmd] of [['Install', installCommand], ['Build', buildCommand], ['Start', startCommand]]) {
                if (cmd) {
    ...
    try {
        ...
        await rsyncFromRemote(sourceHost, Number(sourcePort), sshUser, cred, src, docRoot + '/', ['--exclude=node_modules', '--exclude=.git']);
        await execPromise(`sudo chown -R ${shellEscape(username)}:${shellEscape(username)} ${shellEscape(docRoot)}`);
        await execPromise(`sudo setfacl -R -m user:jonathan:rwx ${shellEscape(docRoot)}`).catch(() => { });
...
async function handleCleanupSiteMigration(payload) {
    ...
    if (username && apps.rows.length > 0)
        await execPromise(`sudo -u ${shellEscape(username)} pm2 save --force`).catch(() => { });
    // 2. nginx vhost.
    await execPromise(`sudo rm -f /etc/nginx/sites-enabled/${shellEscape(domain)} /etc/nginx/sites-available/${shellEscape(domain)}`).catch(() => { });
    await execPromise('sudo nginx -t && sudo systemctl reload nginx').catch(() => { });
...
async function rsyncToRemote(host, port, remoteUser, cred, localPath, remotePath, extraArgs = []) {
    const sshCmd = [
        'ssh', '-o', 'StrictHostKeyChecking=no', '-p', String(port),
        ...(cred.type === 'key' ? ['-i', cred.keyPath] : ['-o', 'PubkeyAuthentication=no']),
    ].join(' ');
           ...
           ON CONFLICT (username) DO UPDATE SET email = EXCLUDED.email RETURNING id`, [username, email, homeDir]);
            ...
            for (const acc of emailAccounts) {
             ...
             VALUES ($1, $2, $3, $4) ON CONFLICT (email) DO NOTHING`, [domainId, emailAddr, mailHash, acc.quota_mb ?? 1024]);
                    ...
                    let remoteMaildir = '';
                    try {
                        remoteMaildir = (await sshRun(remoteHost, remotePort, remoteUser, cred, probe)).trim().split('\n').filter(Boolean)[0] || '';
                    }
                    catch { /* probe failed → treat as none */ }
                    if (remoteMaildir) {
                        try {
                            // Skip stale index files so Dovecot rebuilds them; keep dovecot-uidlist (preserves UIDs + read/unread flags).
                            await rsyncFromRemote(remoteHost, remotePort, remoteUser, cred, remoteMaildir + '/', localMaildir, ['--exclude=dovecot.index*', '--exclude=dovecot.list.index*', '--exclude=dovecot-uidvalidity*']);
