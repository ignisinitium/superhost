...
async function handleGetServicesStatus(taskId) {
    ...
    await client.query('UPDATE tasks SET status = $1, payload = payload || $2, updated_at = NOW() WHERE id = $3', ['completed', JSON.stringify({ status }), taskId]);
}
async function handleManageService(payload) {
