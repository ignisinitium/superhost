...
`;
    ...
    const stripWww = (d: string) => d.replace(/^www\./i, '').trim().toLowerCase();
    const found = new Map<string, string>(); // domain -> docroot (first wins)
    const add = (domain: string, docroot: string) => {
      const d = stripWww(domain);
      if (!d || !d.includes('.') || d === '_' || d.startsWith('~') || d.startsWith('*')) return;
      if (!docroot || !docroot.startsWith('/')) return;
      if (!found.has(d)) found.set(d, docroot.replace(/\/$/, ''));
    };

    // nginx: walk balanced `server { … }` blocks; take all server_names + first server-level root.
    const ng = sec('NGINX');
    for (let i = 0; i < ng.length;) {
      const m = /\bserver\b\s*\{/g; m.lastIndex = i;
      const hit = m.exec(ng);
      if (!hit) break;
      let depth = 0, k = hit.index + hit[0].length - 1;
      for (; k < ng.length; k++) { if (ng[k] === '{') depth++; else if (ng[k] === '}') { depth--; if (depth === 0) { k++; break; } } }
      ...
      if (rootM) for (const n of names) add(n, rootM[1]!.trim());
    }
    ...
    const stackByRoot = new Map<string, string>();
    if (docroots.length) {
...
`;
      ...
      for (const line of det.split('\n')) {
        const t = line.split('\t');
        if (t.length >= 2) stackByRoot.set(t[0]!.trim(), t[1]!.trim());
      }
    }
...
async function handleCleanupSiteMigration(payload: any): Promise<void> {
  ...
  if (mig.migrated_db_name) {
    ...
    if (dbUser) await handleDeleteDatabase({ dbName: mig.migrated_db_name, dbUser }).catch((e) => siteLog(migrationId, `DB drop warning: ${e.message}`));
  ...
  if (!didRemoveUser) {
    if (username) await execPromise(`sudo rm -rf /home/${shellEscape(username)}/public_html/${shellEscape(domain)}`).catch(() => {});
