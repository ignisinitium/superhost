...
const defaultForm = (): Omit<HostingPackage, 'id' | 'created_at' | 'updated_at'> => ({
  name: '',
  description: '',
  price_cents: 0,
  setup_fee_cents: 0,
...
const PackagesPage: React.FC = () => {
  ...
  const createMutation = useMutation({
    ...
    onError: (err: any) => toast.error(err.response?.data?.message || 'Failed to create package'),
  });
  ...
  const updateMutation = useMutation({
    ...
    onError: (err: any) => toast.error(err.response?.data?.message || 'Failed to update package'),
  });
  ...
  const deleteMutation = useMutation({
    ...
    onError: (err: any) => toast.error(err.response?.data?.message || 'Failed to delete package'),
  });
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Hosting Packages</h1>
          <p className="text-slate-500 mt-1">Define plans and add-ons with full resource limits for your clients.</p>
        </div>
        <button
          onClick={openCreate}
      ...
      <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
        <div className="p-5 border-b border-slate-100 bg-slate-50/50 flex items-center gap-3">
          ...
          <span className="ml-auto text-xs text-slate-400 font-medium">{packages?.length ?? 0} packages</span>
        </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-start justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl border border-slate-200 my-6 animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-8">
              ...
              {/* ── 1. Identity ── */}
              <section>
                <SectionHeader icon={<Tag size={15} />} title="Package Identity" />
...
// Lucide doesn't export Tag separately in some versions — alias it
