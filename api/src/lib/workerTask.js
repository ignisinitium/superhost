 ...
 */
export async function runWorkerTask(command, payload, opts = {}) {
    const timeoutMs = opts.timeoutMs ?? 15000;
    const pollMs = opts.pollMs ?? 200;
    const ins = await query('INSERT INTO tasks (command, payload) VALUES ($1, $2) RETURNING id, command, payload, status, created_at', [command, JSON.stringify(payload)]);
    ...
    while (Date.now() < deadline) {
        ...
        const row = r.rows[0];
        if (!row)
            throw new Error('Task record disappeared');
        if (row.status === 'completed') {
            return (row.payload?.result ?? null);
        }
