...
export const checkIpBlock = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    const result = await query(
      'SELECT * FROM blocked_ips WHERE ip_address = $1 AND (expires_at IS NULL OR expires_at > NOW())',
      [ip]
    );
  ...
  try {
    ...
    if (!success) {
      ...
      if (failCount >= FAIL_THRESHOLD) {
        ...
        await query(
          'INSERT INTO blocked_ips (ip_address, reason, expires_at) VALUES ($1, $2, NOW() + INTERVAL \'24 hours\') ON CONFLICT DO NOTHING',
          [ip, 'Automatic block: Multiple failed login attempts']
        );
