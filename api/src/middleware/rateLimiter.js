...
async function getSettings() {
    ...
    if (_cache && now - _cacheAt < CACHE_TTL)
        return _cache;
    try {
        const res = await query(`SELECT key, value FROM server_settings
...
export const checkIpBlock = async (req, res, next) => {
    ...
    try {
        const result = await query('SELECT reason FROM blocked_ips WHERE ip_address = $1 AND (expires_at IS NULL OR expires_at > NOW())', [ip]);
...
export const logLoginAttempt = async (ip, username, success) => {
    try {
        ...
        const countRes = await query(`SELECT count(*) FROM login_attempts
       WHERE ip_address = $1 AND success = false
         AND created_at > NOW() - ($2 * INTERVAL '1 minute')`, [ip, windowMinutes]);
        const failCount = parseInt(countRes.rows[0].count, 10);
        if (failCount >= failThreshold) {
            ...
            await query(`INSERT INTO blocked_ips (ip_address, reason, expires_at)
         VALUES (
           $1, $2,
         ...
         ON CONFLICT (ip_address) DO UPDATE
           SET reason     = EXCLUDED.reason,
               ...
               created_at = NOW()`, [ip, 'Automatic block: multiple failed login attempts', banMinutes]);
