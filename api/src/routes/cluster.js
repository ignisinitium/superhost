...
router.post('/nodes', async (req, res) => {
    ...
    try {
        const result = await query('INSERT INTO cluster_nodes (hostname, ip_address, role, ssh_port) VALUES ($1, $2, $3, $4) RETURNING *', [hostname, ipAddress, role || 'edge', sshPort || 22]);
        // Queue a health check task
        await query('INSERT INTO tasks (command, payload) VALUES ($1, $2)', ['CHECK_NODE_HEALTH', { nodeId: result.rows[0].id, ipAddress }]);
...
router.post('/sync-all', async (req, res) => {
    try {
        ...
        for (const node of nodes.rows) {
            await query('INSERT INTO tasks (command, payload) VALUES ($1, $2)', ['SYNC_CLUSTER_CONFIG', { nodeId: node.id, ipAddress: node.ip_address }]);
        }
