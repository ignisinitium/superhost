...
router.post('/', async (req: AuthRequest, res) => {
  ...
  try {
    ...
    const result = await query(
      'INSERT INTO user_cron_jobs (user_id, minute, hour, day, month, weekday, command, description) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *',
      [userId || null, minute || '*', hour || '*', day || '*', month || '*', weekday || '*', command, description]
    );
    ...
    await query(
      ...
      ['SYNC_CRONTAB', { username, jobs: allJobs.rows }]
    );
...
router.delete('/:id', async (req: AuthRequest, res) => {
  ...
  try {
    ...
    if (jobRes.rowCount === 0) return res.status(404).json({ message: 'Cron job not found' });
...
router.put('/:id', async (req: AuthRequest, res) => {
  ...
  try {
    ...
    const result = await query(
      ...
      [userId || null, minute || '*', hour || '*', day || '*', month || '*', weekday || '*', command, description, id]
    );
