...
const stripe = new Stripe(process.env.STRIPE_SECRET_KEY || 'sk_test_dummy_key', {
    apiVersion: '2023-10-16', // Cast to avoid TS error on older/newer versions depending on types
});
...
router.post('/products', authenticateAdmin, async (req, res) => {
    ...
    try {
        const result = await query('INSERT INTO products (name, description, price_cents, type, stripe_price_id) VALUES ($1, $2, $3, $4, $5) RETURNING *', [name, description, price_cents, type, stripe_price_id]);
...
router.post('/create-checkout-session', async (req, res) => {
    ...
    try {
        ...
        if (!process.env.STRIPE_SECRET_KEY) {
            ...
            await query('INSERT INTO invoices (user_id, stripe_invoice_id, amount_cents, status) VALUES ($1, $2, $3, $4)', [userId, `demo_inv_${Date.now()}`, product.price_cents, 'open']);
