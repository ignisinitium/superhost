    ...
    try {
        switch (event.type) {
            ...
            case 'invoice.paid': {
                ...
                if (invoice.customer) {
                    const reactivated = await query(`UPDATE users SET status = 'active', subscription_status = 'active'
             WHERE stripe_customer_id = $1 AND status = 'suspended'
             RETURNING id, username`, [invoice.customer]);
                    for (const u of reactivated.rows) {
                        await query('INSERT INTO tasks (command, payload) VALUES ($1, $2)', ['UNSUSPEND_ACCOUNT', { userId: u.id, username: u.username }]);
                    }
                }
                break;
            }
           ...
           WHERE stripe_subscription_id = $3 OR stripe_customer_id = $4`, [sub.status, periodEnd, sub.id, sub.customer]);
            ...
            case 'customer.subscription.deleted': {
                ...
                const suspended = await query(`UPDATE users SET status = 'suspended', subscription_status = 'canceled'
           WHERE stripe_subscription_id = $1 OR stripe_customer_id = $2
           RETURNING id, username`, [sub.id, sub.customer]);
                for (const u of suspended.rows) {
                    await query('INSERT INTO tasks (command, payload) VALUES ($1, $2)', ['SUSPEND_ACCOUNT', { userId: u.id, username: u.username }]);
                }
...
router.get('/signups', authenticateAdmin, async (_req, res) => {
    try {
        const result = await query(`SELECT s.id, s.username, s.email, s.primary_domain, s.billing_cycle, s.amount_cents,
...
router.post('/signups/:id/provision', authenticateAdmin, async (req, res) => {
    try {
        await provisionSignupById(parseInt(req.params.id, 10));
...
router.get('/inquiries', authenticateAdmin, async (_req, res) => {
    try {
        const result = await query(`SELECT i.*, p.name AS service_name FROM service_inquiries i
...
router.post('/create-checkout-session', async (req, res) => {
    ...
    try {
        ...
        const prodRes = await query('SELECT name, price_cents, annual_price_cents, stripe_price_id FROM products WHERE id = $1 AND is_active = TRUE', [productId]);
        if (prodRes.rows.length === 0)
            return res.status(404).json({ message: 'Package not found' });
        ...
        if (!process.env.STRIPE_SECRET_KEY) {
            await query('INSERT INTO invoices (user_id, product_id, stripe_invoice_id, amount_cents, status) VALUES ($1, $2, $3, $4, $5)', [userId, productId, `demo_inv_${Date.now()}`, billedAmount, 'open']);
