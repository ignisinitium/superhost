...
router.post('/create-checkout-session', async (req, res) => {
    ...
    try {
        ...
        const prodRes = await query('SELECT name, price_cents, stripe_price_id FROM products WHERE id = $1 AND is_active = TRUE', [productId]);
        if (prodRes.rows.length === 0)
            return res.status(404).json({ message: 'Package not found' });
        const product = prodRes.rows[0];
        // Demo mode
        if (!process.env.STRIPE_SECRET_KEY) {
            await query('INSERT INTO invoices (user_id, product_id, stripe_invoice_id, amount_cents, status) VALUES ($1, $2, $3, $4, $5)', [userId, productId, `demo_inv_${Date.now()}`, product.price_cents, 'open']);
