...
async function generateDovecotPassword(plain) {
...
router.get('/domains', async (req, res) => {
    ...
    try {
        const result = await query('SELECT id, domain_name FROM domains WHERE user_id = $1 ORDER BY domain_name ASC', [userId]);
        res.json(result.rows);
    }
    catch (err) {
        res.status(500).json({ message: err.message });
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        if (existingMd.rows.length === 0) {
            const ins = await query('INSERT INTO mail_domains (domain_name, user_id) VALUES ($1, $2) RETURNING id', [domainName, userId]);
            mailDomainId = ins.rows[0].id;
        }
        else {
            mailDomainId = existingMd.rows[0].id;
        }
        const hash = await generateDovecotPassword(password);
        const result = await query('INSERT INTO mail_users (domain_id, email, password_hash, quota) VALUES ($1, $2, $3, $4) RETURNING id, email, quota', [mailDomainId, fullEmail, hash, quota]);
...
router.get('/forwarders', async (req, res) => {
    ...
    try {
        const result = userId
            ? await query(`SELECT mf.*, md.domain_name, u.username as owner
           FROM mail_forwarders mf
           JOIN mail_domains md ON mf.domain_id = md.id
           JOIN users u ON md.user_id = u.id
           WHERE md.user_id = $1
           ORDER BY mf.source ASC`, [userId])
            : await query(`SELECT mf.*, md.domain_name, u.username as owner
           FROM mail_forwarders mf
           JOIN mail_domains md ON mf.domain_id = md.id
           JOIN users u ON md.user_id = u.id
