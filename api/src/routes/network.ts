...
router.post('/ips', async (req, res) => {
  ...
  try {
    // Save to DB
    const result = await query(
      'INSERT INTO virtual_ips (ip_address, interface) VALUES ($1, $2) RETURNING *',
      [ipAddress, interfaceName || 'eth0:1']
    );

    // Tell worker to bring interface up
    await query(
      'INSERT INTO tasks (command, payload) VALUES ($1, $2)',
      ['ADD_VIRTUAL_IP', { ipAddress, interface: interfaceName || 'eth0:1' }]
    );
...
router.delete('/ips/:id', async (req, res) => {
  ...
  try {
    ...
    if (ipRes.rows.length === 0) return res.status(404).json({ message: 'IP not found' });
    ...
    await query(
      'INSERT INTO tasks (command, payload) VALUES ($1, $2)',
      ['REMOVE_VIRTUAL_IP', { ipAddress: ip_address, interface: iface }]
    );
...
router.post('/ips/:id/assign', async (req, res) => {
  ...
  try {
    ...
    if (domainName) {
      ...
      await query(
        'INSERT INTO tasks (command, payload) VALUES ($1, $2)',
        ['ASSIGN_VIRTUAL_IP', { domainName, ipAddress: domainId ? ipAddress : null }]
      );
    }
