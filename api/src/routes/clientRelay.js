...
async function ownsRelay(req, id) {
...
router.get('/', async (req, res) => {
    try {
        const domains = await query('SELECT * FROM mail_relay_domains WHERE user_id = $1 ORDER BY domain_name', [req.userId]);
        const recips = await query(`SELECT r.* FROM mail_relay_recipients r
       JOIN mail_relay_domains d ON r.relay_domain_id = d.id WHERE d.user_id = $1`, [req.userId]);
        const byDomain = {};
        for (const r of recips.rows)
            (byDomain[r.relay_domain_id] ??= []).push(r);
        res.json(domains.rows.map((d) => ({ ...d, recipients: byDomain[d.id] ?? [] })));
    }
    catch (err) {
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        if (dup.rowCount)
            return res.status(409).json({ message: 'That domain is already configured' });
        const r = await query(`INSERT INTO mail_relay_domains (user_id, domain_name, destination_host, destination_port, spam_threshold)
       VALUES ($1, $2, $3, $4, $5) RETURNING *`, [req.userId, dom, host, port, spamThreshold ?? 5.0]);
        await reconfigure();
...
router.patch('/:id', async (req, res) => {
    ...
    try {
        if (!(await ownsRelay(req, id)))
            return res.status(404).json({ message: 'Not found' });
        if (destinationHost !== undefined && !HOST_RE.test(String(destinationHost))) {
            return res.status(400).json({ message: 'Invalid destination mail server' });
        }
        const r = await query(`UPDATE mail_relay_domains SET
       ...
       WHERE id = $5 RETURNING *`, [destinationHost ?? null, destinationPort ?? null, spamThreshold ?? null, enabled ?? null, id]);
        await reconfigure();
        res.json(r.rows[0]);
    }
    catch (err) {
...
router.post('/:id/recipients', async (req, res) => {
    ...
    try {
        if (!(await ownsRelay(req, id)))
            return res.status(404).json({ message: 'Not found' });
        const r = await query(`INSERT INTO mail_relay_recipients (relay_domain_id, address) VALUES ($1, $2)
...
router.delete('/:id/recipients/:rid', async (req, res) => {
...
router.get('/quarantine', async (req, res) => {
    try {
        const r = await query(`SELECT q.id, q.recipient, q.sender, q.subject, q.spam_score, q.created_at, d.domain_name
...
async function ownsQuarantine(req, id) {
    const r = await query(`SELECT 1 FROM mail_relay_quarantine q JOIN mail_relay_domains d ON q.relay_domain_id = d.id
     WHERE q.id = $1 AND d.user_id = $2`, [id, req.userId]);
    return (r.rowCount ?? 0) > 0;
}
router.post('/quarantine/:qid/release', async (req, res) => {
    const { qid } = req.params;
    try {
        if (!(await ownsQuarantine(req, qid)))
            return res.status(404).json({ message: 'Not found' });
...
router.delete('/quarantine/:qid', async (req, res) => {
    const { qid } = req.params;
    try {
        if (!(await ownsQuarantine(req, qid)))
            return res.status(404).json({ message: 'Not found' });
...
//# sourceMappingURL=clientRelay.js.map