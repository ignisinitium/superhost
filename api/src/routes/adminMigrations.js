...
router.get('/', async (_req, res) => {
    try {
        const result = await query(`SELECT id, remote_host, remote_port, remote_user, status,
...
router.post('/test-connection', async (req, res) => {
    ...
    if (!remoteHost)
        return res.status(400).json({ message: 'Remote host is required' });
    ...
    try {
        const taskRes = await query(`INSERT INTO tasks (command, payload) VALUES ($1, $2) RETURNING id`, ['TEST_SSH_CONNECTION', { remoteHost, remotePort: Number(remotePort), remoteUser, authType, sshPassword, sshKey }]);
...
router.post('/discover', async (req, res) => {
    ...
    if (authType === 'key' && !sshKey)
        return res.status(400).json({ message: 'SSH private key content is required' });
    try {
        const migRes = await query(`INSERT INTO cwp_migrations (remote_host, remote_port, remote_user, status)
       VALUES ($1, $2, $3, 'discovering') RETURNING id`, [remoteHost, Number(remotePort), remoteUser]);
        const migrationId = migRes.rows[0].id;
...
router.post('/:id/retry-discover', async (req, res) => {
    ...
    try {
        ...
        await query(`UPDATE cwp_migrations SET status = 'discovering', error_message = NULL, logs = '[]', updated_at = NOW() WHERE id = $1`, [mig.id]);
        await query('INSERT INTO tasks (command, payload) VALUES ($1, $2)', [
            'DISCOVER_CWP',
...
router.post('/:id/migrate', async (req, res) => {
    ...
    try {
        ...
        const mig = migRes.rows[0];
        if (mig.status !== 'ready')
            return res.status(400).json({ message: `Migration is in '${mig.status}' state, expected 'ready'` });
        await query(`UPDATE cwp_migrations SET status = 'migrating', selected_users = $1, updated_at = NOW() WHERE id = $2`, [selectedUsers, mig.id]);
        await query('INSERT INTO tasks (command, payload) VALUES ($1, $2)', [
            'MIGRATE_CWP',
            {
                migrationId: mig.id,
