...
router.post('/zones/:zoneId/records', async (req, res) => {
    ...
    try {
        ...
        // 2. Insert record
        const result = await query('INSERT INTO dns_records (zone_id, name, type, content, priority, ttl) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *', [zoneId, name, type.toUpperCase(), content, priority || null, ttl || null]);
        // 3. Trigger worker to sync zone
        await query('INSERT INTO tasks (command, payload) VALUES ($1, $2)', ['SYNC_DNS_ZONE', { zoneId, domainName: domain_name }]);
...
router.put('/zones/:zoneId/records/:recordId', async (req, res) => {
    ...
    try {
        ...
        const { domain_name } = zoneRes.rows[0];
        // 2. Update record
        const result = await query('UPDATE dns_records SET name = $1, type = $2, content = $3, priority = $4, ttl = $5 WHERE id = $6 AND zone_id = $7 RETURNING *', [name, type.toUpperCase(), content, priority || null, ttl || null, recordId, zoneId]);
