 ...
 */
function sanitizeFilePath(inputPath, username) {
...
router.get('/list', async (req, res) => {
    ...
    try {
        ...
        const taskRes = await query('INSERT INTO tasks (command, payload) VALUES ($1, $2) RETURNING id', ['LIST_FILES', { username, path: safePath }]);
...
router.get('/read', async (req, res) => {
    ...
    try {
        ...
        const username = userRes.rows[0].username;
        const safePath = sanitizeFilePath(inputPath, username);
        const taskRes = await query('INSERT INTO tasks (command, payload) VALUES ($1, $2) RETURNING id', ['READ_FILE_CONTENT', { username, filePath: safePath }]);
        res.json({ taskId: taskRes.rows[0].id });
    }
    catch (err) {
        res.status(400).json({ message: err.message });
...
router.post('/write', async (req, res) => {
    ...
    try {
        ...
        if (content.length > 10 * 1024 * 1024)
            return res.status(413).json({ message: 'File content too large (max 10 MB)' });
        const taskRes = await query('INSERT INTO tasks (command, payload) VALUES ($1, $2) RETURNING id', ['WRITE_FILE_CONTENT', { username, filePath: safePath, content }]);
        res.json({ taskId: taskRes.rows[0].id });
    }
    catch (err) {
        res.status(400).json({ message: err.message });
...
router.delete('/delete', async (req, res) => {
    ...
    try {
        ...
        const username = userRes.rows[0].username;
        const safePath = sanitizeFilePath(inputPath, username);
        const taskRes = await query('INSERT INTO tasks (command, payload) VALUES ($1, $2) RETURNING id', ['DELETE_FILE', { username, filePath: safePath }]);
        res.json({ taskId: taskRes.rows[0].id });
    }
    catch (err) {
        res.status(400).json({ message: err.message });
