...
router.post('/webhook/:token', async (req, res) => {
  ...
  try {
    ...
    if (repoRes.rows.length === 0) return res.status(404).json({ message: 'Invalid token' });
...
router.get('/', async (req: AuthRequest, res) => {
  try {
    const result = await query(
      'SELECT r.*, d.domain_name FROM user_git_repos r JOIN domains d ON r.domain_id = d.id WHERE r.user_id = $1',
      [req.userId]
    );
...
router.post('/', async (req: AuthRequest, res) => {
  ...
  try {
    ...
    const result = await query(
      'INSERT INTO user_git_repos (user_id, domain_id, repo_url, branch, deploy_path, webhook_token) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *',
      [req.userId, domainId, repoUrl, branch || 'main', deployPath || '', webhookToken]
    );
