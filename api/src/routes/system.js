...
router.post('/reboot', async (_req, res) => {
    try {
        const taskRes = await query("INSERT INTO tasks (command, payload) VALUES ($1, $2) RETURNING id", ['REBOOT_SERVER', {}]);
...
router.post('/restart-web', async (_req, res) => {
    try {
        const taskRes = await query("INSERT INTO tasks (command, payload) VALUES ($1, $2) RETURNING id", ['RESTART_WEB_SERVICES', {}]);
        res.json({ taskId: taskRes.rows[0].id });
    }
    catch (err) {
...
router.post('/exec', async (req, res) => {
    ...
    try {
        const taskRes = await query("INSERT INTO tasks (command, payload) VALUES ($1, $2) RETURNING id", ['EXEC_COMMAND', { command: command.trim() }]);
        res.json({ taskId: taskRes.rows[0].id });
    }
    catch (err) {
...
router.post('/backup', async (_req, res) => {
    try {
        const taskRes = await query("INSERT INTO tasks (command, payload) VALUES ($1, $2) RETURNING id", ['ADMIN_BACKUP', {}]);
        res.json({ taskId: taskRes.rows[0].id });
    }
    catch (err) {
