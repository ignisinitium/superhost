...
async function accountType(packageId: number | null | undefined): Promise<string> {
  if (!packageId) return 'hosting';
...
router.get('/set-password/validate', async (req, res) => {
  ...
  try {
    const r = await query(
      `SELECT u.username FROM password_setup_tokens t
...
router.post('/verify-2fa', checkIpBlock, async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length === 0) return res.status(401).json({ message: 'Invalid account' });
    const user = result.rows[0];
...
router.post('/logout', authenticateClient, async (req: AuthRequest, res) => {
  try {
...
router.post('/setup-2fa', authenticateClient, async (req: AuthRequest, res) => {
  try {
...
router.post('/enable-2fa', authenticateClient, async (req: AuthRequest, res) => {
  const { token } = req.body;
  try {
    const userRes = await query('SELECT totp_secret FROM users WHERE id = $1', [req.userId]);
    const secret = userRes.rows[0]?.totp_secret;
    if (!secret) return res.status(400).json({ message: 'Run setup first' });
...
router.post('/disable-2fa', authenticateClient, async (req: AuthRequest, res) => {
