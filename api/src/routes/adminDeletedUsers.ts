...
router.get('/', async (_req, res) => {
  try {
    const result = await query(
      `SELECT id, username, email, original_user_id, archive_path,
...
router.post('/:id/restore', async (req, res) => {
  ...
  try {
    ...
    if (deletedRes.rows.length === 0) return res.status(404).json({ message: 'Deleted user not found' });

    const taskRes = await query(
      'INSERT INTO tasks (command, payload) VALUES ($1, $2) RETURNING id',
      ['RESTORE_USER', { deletedUserId: parseInt(id) }]
    );
...
router.delete('/:id', async (req, res) => {
  ...
  try {
    ...
    await query(
      ...
      ['PURGE_USER_ARCHIVE', { deletedUserId: parseInt(id), archivePath: deletedRes.rows[0].archive_path }]
    );
