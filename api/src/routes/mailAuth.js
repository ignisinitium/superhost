...
function opensslCrypt(flag, salt, password) {
...
async function verifyMailPassword(password, stored) {
    if (!stored)
        return false;
    ...
    if (/^\{(PLAIN|CLEARTEXT)\}/i.test(stored))
        return password === raw;
    return false;
}
router.post('/login', checkIpBlock, async (req, res) => {
    ...
    `, [email.toLowerCase().trim()]);
        ...
        const match = await verifyMailPassword(password, mu.password_hash);
        if (!match) {
            await logLoginAttempt(ip, email, false);
            return res.status(401).json({ message: 'Invalid email or password' });
        }
