...
router.get('/:id/subdomains', async (req: AuthRequest, res) => {
  const { id } = req.params;
  try {
    const parent = await query('SELECT id FROM domains WHERE id = $1 AND user_id = $2', [id, req.userId]);
    if (parent.rowCount === 0) return res.status(404).json({ message: 'Domain not found' });
    const result = await query(
      'SELECT * FROM domains WHERE parent_domain_id = $1 AND user_id = $2 ORDER BY domain_name',
      [id, req.userId],
    );
    res.json(result.rows);
  } catch (err) {
...
router.post('/:id/subdomains', async (req: AuthRequest, res) => {
  const { id } = req.params;
  ...
  try {
    ...
    const parentRes = await query(
      'SELECT d.domain_name, u.username, u.package_id FROM domains d JOIN users u ON d.user_id = u.id WHERE d.id = $1 AND d.user_id = $2 AND d.is_subdomain = FALSE',
      [id, req.userId],
    );
    if (parentRes.rows.length === 0) return res.status(404).json({ message: 'Parent domain not found' });
    ...
    if (dup.rows.length > 0) return res.status(400).json({ message: 'Subdomain already exists' });

    const result = await query(
      `INSERT INTO domains (user_id, domain_name, document_root, php_version, parent_domain_id, is_subdomain)
       VALUES ($1, $2, $3, '8.3', $4, TRUE) RETURNING *`,
      [req.userId, fullName, docRoot, id],
    );
    const domain = result.rows[0];

    const taskRes = await query(
      'INSERT INTO tasks (command, payload) VALUES ($1, $2) RETURNING id',
      ['CREATE_DOMAIN', { domainId: domain.id, domainName: fullName, username, phpVersion: '8.3', docRoot }],
    );
...
// Delete a subdomain
router.delete('/subdomains/:id', async (req: AuthRequest, res) => {
  const { id } = req.params;
  try {
    const subRes = await query(
      'SELECT d.domain_name, u.username FROM domains d JOIN users u ON d.user_id = u.id WHERE d.id = $1 AND d.user_id = $2 AND d.is_subdomain = TRUE',
      [id, req.userId],
    );
    if (subRes.rows.length === 0) return res.status(404).json({ message: 'Subdomain not found' });
    ...
    await query(
      ...
      ['DELETE_DOMAIN', { domainName: domain_name, username }],
    );
...
router.post('/:id/install-ssl', async (req: AuthRequest, res) => {
  const { id } = req.params;
