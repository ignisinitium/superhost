...
async function usernameTaken(username) {
    ...
    if (u.rowCount)
        return true;
    const p = await query(`SELECT 1 FROM pending_signups WHERE lower(username) = lower($1)
       AND status = 'pending' AND created_at > NOW() - INTERVAL '1 day'`, [username]);
    return (p.rowCount ?? 0) > 0;
}
// Public plans for the marketing/pricing page.
router.get('/plans', async (_req, res) => {
    try {
        const result = await query(`SELECT id, name, description, price_cents, annual_price_cents, type,
...
router.get('/services', async (_req, res) => {
    try {
        const result = await query(`SELECT id, name, description, price_cents, annual_price_cents, onetime_price_cents, is_custom, billing_unit, sort_order,
...
router.get('/portfolio', async (_req, res) => {
    try {
        const result = await query(`SELECT id, title, url, description, category, image_url
...
router.post('/inquiry', checkIpBlock, async (req, res) => {
    ...
    if (!message || String(message).trim().length < 5)
        return res.status(400).json({ message: 'Please describe your project' });
    try {
        let pid = null;
        if (productId) {
        ...
        await query(`INSERT INTO service_inquiries (product_id, name, email, phone, company, budget, message)
       VALUES ($1, $2, $3, $4, $5, $6, $7)`, [pid, String(name).slice(0, 120), String(email).slice(0, 120),
...
router.post('/checkout', checkIpBlock, async (req, res) => {
    ...
    try {
        ...
        const prodRes = await query("SELECT * FROM products WHERE id = $1 AND is_active = TRUE AND type IN ('hosting','reseller','service')", [productId]);
        ...
        if (primaryDomain) {
            ...
            if (dom.rowCount)
                return res.status(409).json({ message: 'That domain is already hosted here' });
        }
        ...
        await query(`INSERT INTO pending_signups
         (session_token, username, email, password_hash, primary_domain, product_id, billing_cycle, amount_cents)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`, [token, uname, mail, passwordHash, primaryDomain || null, product.id, billingCycle, amount]);
        ...
        try {
            ...
            return res.status(502).json({ message: `Payment processor not configured: ${err.message}` });
        }
...
router.post('/checkout-filter', checkIpBlock, async (req, res) => {
    ...
    const addrList = Array.isArray(addresses)
        ? addresses.map((a) => String(a).toLowerCase().trim()).filter(Boolean) : [];
    try {
        if (!USERNAME_RE.test(uname) || RESERVED.has(uname))
        ...
        if (addrList.length < 1)
            return res.status(400).json({ message: 'Add at least one mailbox to protect' });
        ...
        if (emailExists.rowCount)
            return res.status(409).json({ message: 'An account with that email already exists' });
        const domTaken = await query('SELECT 1 FROM mail_relay_domains WHERE lower(domain_name)=lower($1) UNION SELECT 1 FROM domains WHERE lower(domain_name)=lower($1)', [dom]);
        if (domTaken.rowCount)
            return res.status(409).json({ message: 'That domain is already configured here' });
       ...
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,'filter',$9,$10,$11,$12)`, [token, uname, mail, passwordHash, dom, product.id, billingCycle, unit * quantity, host, port, addrList.join(','), quantity]);
        const origin = safeOrigin(req);
        let session;
        try {
            session = await stripe.checkout.sessions.create({
