...
router.get('/settings', async (_req, res) => {
    try {
        ...
        const taskRes = await query("INSERT INTO tasks (command, payload) VALUES ('GET_BIND_STATUS', '{}') RETURNING id");
...
router.put('/settings', async (req, res) => {
    const { ns1, ns2, master_domain, server_ip } = req.body;
    const updates = [];
    if (ns1)
        updates.push(['ns1', ns1]);
    if (ns2)
        updates.push(['ns2', ns2]);
    if (master_domain)
        updates.push(['master_domain', master_domain]);
    if (server_ip)
        updates.push(['server_ip', server_ip]);
    if (updates.length === 0)
        return res.status(400).json({ message: 'No fields to update' });
    try {
        for (const [key, value] of updates) {
            await query("INSERT INTO server_settings (key, value, updated_at) VALUES ($1, $2, NOW()) ON CONFLICT (key) DO UPDATE SET value=$2, updated_at=NOW()", [key, value]);
        }
...
router.post('/bind/:action', async (req, res) => {
    ...
    try {
        const taskRes = await query("INSERT INTO tasks (command, payload) VALUES ('MANAGE_BIND', $1) RETURNING id", [JSON.stringify({ action })]);
...
//# sourceMappingURL=adminNameservers.js.map