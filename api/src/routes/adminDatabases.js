...
router.get('/', async (req, res) => {
    try {
        ...
        const result = userId
            ? await query('SELECT d.*, u.username as owner_name FROM databases d JOIN users u ON d.user_id = u.id WHERE d.user_id = $1 ORDER BY d.created_at DESC', [userId])
            : await query('SELECT d.*, u.username as owner_name FROM databases d JOIN users u ON d.user_id = u.id ORDER BY d.created_at DESC');
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        const taskRes = await query('INSERT INTO tasks (command, payload) VALUES ($1, $2) RETURNING id', ['CREATE_DATABASE', { dbName: fullDbName, dbUser: fullDbUser, dbPassword }]);
...
router.put('/:id/password', async (req, res) => {
    ...
    try {
        ...
        const taskRes = await query('INSERT INTO tasks (command, payload) VALUES ($1, $2) RETURNING id', ['CHANGE_DB_PASSWORD', { dbUser: dbRes.rows[0].db_user, newPassword }]);
...
router.delete('/:id', async (req, res) => {
    ...
    try {
        ...
        const taskRes = await query('INSERT INTO tasks (command, payload) VALUES ($1, $2) RETURNING id', ['DELETE_DATABASE', { dbName: db_name, dbUser: db_user }]);
