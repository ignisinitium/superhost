...
router.get('/quarantine/:id/message', async (req, res) => {
  ...
  try {
    ...
    if (exists.rowCount === 0) return res.status(404).json({ message: 'Not found' });
...
router.post('/quarantine/:id/block', async (req, res) => {
  ...
  try {
    const qRes = await query(
      'SELECT id, file_path, sender, mail_user_id FROM mail_quarantine WHERE id = $1', [id],
    );
    if (qRes.rows.length === 0) return res.status(404).json({ message: 'Not found' });
    const item = qRes.rows[0];
    if (item.sender) {
    ...
    if (item.file_path) {
      await query('INSERT INTO tasks (command, payload) VALUES ($1, $2)',
        ['DELETE_FILE', { filePath: item.file_path }]);
    }
