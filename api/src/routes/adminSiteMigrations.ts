...
router.post('/scan-import', async (req: AuthRequest, res) => {
  ...
  try {
    ...
    for (const s of sites) {
      ...
      if (!DOMAIN_RE.test(dom) || !rp.startsWith('/')) { skipped.push(dom || '(invalid)'); continue; }
      ...
      const mig = await query(
        `INSERT INTO site_migrations
         ...
         VALUES ('pull',$1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING id`,
        [host, port, sshUser, rp, userId, dom, st, cmds.installCommand, cmds.startCommand]);
