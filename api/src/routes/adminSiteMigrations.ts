...
router.get('/', async (_req, res) => {
  try {
    const r = await query(
      `SELECT m.id, m.source_host, m.ssh_user, m.remote_path, m.domain_name, m.stack,
...
router.post('/test-connection', async (req, res) => {
  ...
  try {
    const t = await query(
      ...
      ['TEST_SSH_CONNECTION', { host, port: parseInt(port ?? 22, 10), user, authType, sshPassword, sshKey }]);
...
router.post('/', async (req: AuthRequest, res) => {
  const {
    sourceHost, sourcePort, sshUser, authType, sshPassword, sshKey, remotePath,
  ...
  try {
    if (!HOST_RE.test(host)) return res.status(400).json({ message: 'Invalid source host' });
    ...
    const mig = await query(
      ...
      [host, port, sshUser, remotePath, userId, dom, st,
