...
router.get('/', async (req, res) => {
    try {
        const result = await query('SELECT a.*, d.domain_name FROM user_apps a JOIN domains d ON a.domain_id = d.id WHERE a.user_id = $1 ORDER BY a.created_at DESC', [req.userId]);
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        while (usedPorts.includes(port))
            port++;
        // 3. Save to DB
        const result = await query('INSERT INTO user_apps (user_id, domain_id, name, type, port, startup_script) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *', [userId, domainId, name, type, port, startupScript]);
        // 4. Get username
        const userRes = await query('SELECT username FROM users WHERE id = $1', [userId]);
        const username = userRes.rows[0].username;
        // 5. Create worker task
        await query('INSERT INTO tasks (command, payload) VALUES ($1, $2)', ['SETUP_APP_RUNTIME', {
...
router.post('/:id/manage', async (req, res) => {
    ...
    try {
        ...
        await query('INSERT INTO tasks (command, payload) VALUES ($1, $2)', ['MANAGE_APP_RUNTIME', { appId: app.id, username: app.username, action, type: app.type, name: app.name }]);
...
router.delete('/:id', async (req, res) => {
    ...
    try {
        ...
        await query('INSERT INTO tasks (command, payload) VALUES ($1, $2)', ['DELETE_APP_RUNTIME', { appId: app.id, username: app.username, domainName: app.domain_name, name: app.name }]);
...
router.post('/install-wordpress', async (req, res) => {
    ...
    try {
        ...
        await query('INSERT INTO databases (user_id, db_name, db_user) VALUES ($1, $2, $3)', [userId, fullDbName, fullDbUser]);
        // 5. Queue worker tasks: Create DB, then Install WP
        // We send a single task that orchestrates both to ensure atomic installation
        const taskRes = await query('INSERT INTO tasks (command, payload) VALUES ($1, $2) RETURNING id', ['INSTALL_WORDPRESS', {
                domainName,
                username,
                dbName: fullDbName,
                dbUser: fullDbUser,
