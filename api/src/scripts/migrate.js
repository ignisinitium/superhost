...
dotenv.config();
async function orchestrateMigration(metadata, sourceHtml) {
    console.log('Detected Metadata:', metadata);
    // A. Create User
    const userRes = await query('INSERT INTO users (username, email, home_dir) VALUES ($1, $2, $3) ON CONFLICT (username) DO UPDATE SET username = $1 RETURNING id', [metadata.username, metadata.email || `${metadata.username}@migrated.com`, `/home/${metadata.username}`]);
    ...
    for (const domain of metadata.domains) {
        await query('INSERT INTO domains (user_id, domain_name, document_root) VALUES ($1, $2, $3) ON CONFLICT DO NOTHING', [userId, domain, `/home/${metadata.username}/public_html/${domain}`]);
        await query('INSERT INTO tasks (command, payload) VALUES ($1, $2)', [
...
async function migrateCPanel(backupPath) {
...
async function migrateCWP(backupPath) {
